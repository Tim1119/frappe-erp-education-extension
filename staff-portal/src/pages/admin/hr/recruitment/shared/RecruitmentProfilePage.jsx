...
export default function RecruitmentProfilePage({config}){
  ...
  return <>
    ...
    {Boolean(config.connectionItems?.length)&&<ConnectionsPanel groups={[{label:config.eyebrow||"Recruitment",items:config.connectionItems.map(item=>({icon:Link2,label:item.label,count:connections[item.key]||0,onClick:()=>nav(`${item.path}${item.path.includes("?")?"&":"?"}${item.filter}=${encodeURIComponent(name)}`)}))}]}/>} 
