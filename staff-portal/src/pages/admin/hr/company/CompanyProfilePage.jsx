...
import { Building2, FileText, FolderKanban, Headphones, Network, PackageCheck, ReceiptText, ShoppingCart, Users } from "lucide-react";
...
export default function CompanyProfilePage() {
  ...
  return <><PageHeader eyebrow="HR" title={company.company_name || company.name} sub={company.abbr} button={<div style={{ display: "flex", gap: 10 }}>{company.can_edit && <button className="btn btn-secondary" onClick={() => navigate(`/dashboard/company/${encodeURIComponent(name)}/edit`)}>Edit</button>}<button className="btn btn-danger" onClick={() => setDeleteOpen(true)}>Delete</button></div>} />
    ...
    <ConnectionsPanel groups={[{ label: "HR", items: [{ icon: Users, label: "Employees", count: connections.employees, onClick: () => navigate(`/dashboard/employees?company=${encodeURIComponent(name)}`) }, { icon: Network, label: "Departments", count: connections.departments, onClick: () => navigate(`/dashboard/departments?company=${encodeURIComponent(name)}`) }] }, { label: "Pre Sales", items: [{ icon: FileText, label: "Quotations", count: connections.quotations, onClick: () => navigate(`/dashboard/quotations?company=${encodeURIComponent(name)}`) }] }, { label: "Orders", items: [{ icon: ShoppingCart, label: "Sales Orders", count: connections.sales_orders, onClick: () => navigate(`/dashboard/sales-orders?company=${encodeURIComponent(name)}`) }, { icon: PackageCheck, label: "Delivery Notes", count: connections.delivery_notes, onClick: () => navigate(`/dashboard/delivery-notes?company=${encodeURIComponent(name)}`) }, { icon: ReceiptText, label: "Sales Invoices", count: connections.sales_invoices, onClick: () => navigate(`/dashboard/sales-invoices?company=${encodeURIComponent(name)}`) }] }, { label: "Support & Projects", items: [{ icon: Headphones, label: "Issues", count: connections.issues, onClick: () => navigate(`/dashboard/issues?company=${encodeURIComponent(name)}`) }, { icon: FolderKanban, label: "Projects", count: connections.projects, onClick: () => navigate(`/dashboard/projects?company=${encodeURIComponent(name)}`) }] }]} />
