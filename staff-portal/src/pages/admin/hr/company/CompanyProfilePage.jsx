...
export default function CompanyProfilePage() {
  ...
  return <><PageHeader eyebrow="HR" title={company.company_name || company.name} sub={company.abbr} button={<div style={{ display: "flex", gap: 10 }}>{company.can_edit && <button className="btn btn-secondary" onClick={() => navigate(`/dashboard/company/${encodeURIComponent(name)}/edit`)}>Edit</button>}{company.can_delete && <button className="btn btn-danger" onClick={() => setDeleteOpen(true)}>Delete</button>}</div>} />
