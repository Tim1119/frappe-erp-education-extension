...
export default function CompaniesPage() {
  ...
  return <>
    ...
    <div className="panel"><div style={{ overflowX: "auto" }}><table className="tbl"><thead><tr><th>Company Name</th><th>Abbreviation</th><th>Country</th><th>Default Currency</th><th>Parent Company</th><th>Type</th><th /></tr></thead><tbody>{rows.map((row) => <tr key={row.name} className="cursor-pointer" onClick={() => navigate(`/dashboard/company/${encodeURIComponent(row.name)}`)}><td><div style={{ display: "flex", alignItems: "center", gap: 9, fontWeight: 550 }}><Building2 size={17} style={{ color: "var(--ink-4)" }} />{row.company_name || row.name}</div></td><td className="muted2">{row.abbr || "—"}</td><td>{row.country || "—"}</td><td>{row.default_currency || "—"}</td><td>{row.parent_company || "—"}</td><td><GroupBadge isGroup={row.is_group} /></td><td onClick={(e) => e.stopPropagation()}><RowActionsMenu onView={() => navigate(`/dashboard/company/${encodeURIComponent(row.name)}`)} onEdit={() => navigate(`/dashboard/company/${encodeURIComponent(row.name)}/edit`)} onDelete={() => setDeleteTarget(row)} /></td></tr>)}{!loading && !rows.length && <tr><td colSpan={7}><EmptyState title="No companies found" sub="No company records available." /></td></tr>}</tbody></table></div><Pager page={page} setPage={setPage} pageSize={20} count={count} /></div>
