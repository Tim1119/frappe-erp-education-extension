...
function Filter({ config, value, values, onChange }) {
  const [options, setOptions] = useState([]); const linked = config.fieldtype === "Dynamic Link" ? values[config.dependsOn] : config.options; const disabled = Boolean(config.dependsOn && !values[config.dependsOn]);
