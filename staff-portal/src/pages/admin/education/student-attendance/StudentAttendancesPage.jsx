...
export default function StudentAttendancesPage() {
  ...
  const [leaveApplicationFilter] = useState(searchParams.get("leave_application") || "");
  ...
  async function load() {
    try {
      ...
      const r = await getStudentAttendanceList({
        ...
        status: statusFilter,
        leave_application: leaveApplicationFilter,
      });
      setRows(r.rows || []);
      setTotal(r.count || 0);
    } catch (err) {
        ...
        description={
          loading
            ...
            : leaveApplicationFilter
              ? `${total} attendance records created from leave application ${leaveApplicationFilter}`
