...
import { PageHeader, Avatar, StatusBadge, EmptyState } from '../../components/ui/Primitives.jsx';
    ...
    <>
      ...
      <div className="panel" style={{ marginBottom: 18 }}>
        ...
        <div className="grid-3" style={{ padding: '4px 18px 20px' }}>
          <Info label="Academic Year" value={group.academic_year ? group.academic_year : "-"} />
          <Info label="Academic Term" value={group.academic_term ? group.academic_term : "-"} />
          <Info label="Group Based On" value={group.group_based_on ? group.group_based_on : "-"} />
          <Info label="Class" value={group.program ? group.program : "-"} />
          <Info label="Course" value={group.course ? group.group_based_on : "-"} />
          <Info label="Batch" value={group.batch ? group.course : "-"} />
          <Info label="Student Category" value={group.student_category ? group.student_category : "-"} />
          <Info label="Max Strength" value={group.max_strength || 'No limit'} />
        </div>
      </div>

      <div className="panel" style={{ marginBottom: 18 }}>
        <SectionHead icon={Users2} title="Students" sub={`${group.students?.length || 0} enrolled`} />
        {!group.students?.length ? (
          <EmptyState icon={Users2} title="No students assigned" />
        ) : (
          <table className="tbl">
            <thead><tr><th>Student</th><th>Roll No.</th><th>Active</th></tr></thead>
            <tbody>
              {group.students.map((s, i) => (
                <tr key={i} className="row">
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <Avatar name={s.student_name || s.student} size={28}    src={s.image}/>
                      <span style={{ fontWeight: 550 }}>{s.student_name || s.student}</span>
                    </div>
                  </td>
                  <td className="tnum muted">{s.group_roll_number || '—'}</td>
                  <td><StatusBadge s={s.active ? 'ACTIVE' : 'INACTIVE'} /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="panel">
        <SectionHead icon={Contact} title="Instructors" sub={`${group.instructors?.length || 0} assigned`} />
        {!group.instructors?.length ? (
          ...
          <div style={{ padding: '4px 18px 18px', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {group.instructors.map((item, i) => (
              <span key={i} className="chip">{item.instructor_name || item.instructor}</span>
            ))}
          </div>
        )}
      </div>
