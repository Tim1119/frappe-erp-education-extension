...
function FeesDashboard() {
  ...
  useEffect(() => {
    frappe
      .getList("Fees", { fields: DOCTYPES.fees.fields, limit_page_length: 500 })
      .then((rows) => {
        ...
        const outstanding = rows.reduce(
          (s, f) => s + Number(f.outstanding_amount || 0),
          0,
        );
  ...
  const rate = totals.billed
    ? ((totals.collected / totals.billed) * 100).toFixed(0)
    : 0;
  ...
  return (
    <div className="grid-stat" style={{ marginBottom: 20 }}>
      <StatCard
        ico={Wallet}
        tone="brand"
        value={`₦${n(totals.billed)}`}
        label="Total Billed"
      />
      <StatCard
        ico={CheckCircle2}
        tone="green"
        value={`₦${n(totals.collected)}`}
...
function StructureTab() {
  const { rows, count, loading } = useDocList("feeStructure", {
          ...
          <tbody>
            {rows.map((f) => (
              <tr key={f.name} className="row">
                ...
                <td className="tnum" style={{ color: "var(--success-ink)" }}>
                  ₦{n(f.paid_amount)}
                </td>
                <td
                  className="tnum"
                  style={{
                    color: Number(f.outstanding_amount)
                      ? "var(--danger-ink)"
                      : "var(--ink-4)",
                  }}
                >
                  ₦{n(f.outstanding_amount)}
                </td>
                <td>
                  <StatusBadge s={STATUS_MAP[f.status] || "GENERATED"} />
                </td>
              </tr>
            ))}
          </tbody>
...
function PaymentsTab() {
  const { rows, loading } = useDocList("paymentEntry", {
    ...
    <div className="panel">
      ...
      {!loading && rows.length === 0 && (
        <EmptyState
          ...
          sub="Payments are recorded in Frappe as Payment Entry documents against Student parties."
        />
      )}
    </div>
  );
  ...
  return (
    <>
      <PageHeader
        eyebrow="Operations"
        title="Fees"
        sub="Frappe Education Fee module"
      />
