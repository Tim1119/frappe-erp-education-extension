...
function safeParseServerMessages(raw) {
  try {
    ...
    return arr.map((m) => { try { return JSON.parse(m).message; } catch { return m; } }).join(' ');
  } catch {
    return raw;
  }
}
...
export async function createDoc(doctype, values) {
  const data = await request(`/api/resource/${encodeURIComponent(doctype)}`, { method: 'POST', body: values });
  return data?.data;
}

export async function updateDoc(doctype, name, values) {
  const data = await request(`/api/resource/${encodeURIComponent(doctype)}/${encodeURIComponent(name)}`, { method: 'PUT', body: values });
  return data?.data;
}

export async function deleteDoc(doctype, name) {
  await request(`/api/resource/${encodeURIComponent(doctype)}/${encodeURIComponent(name)}`, { method: 'DELETE' });
  return true;
}
 ...
 */
export async function getPage(doctype, { fields, filters, order_by, page = 1, pageSize = 20 } = {}) {
