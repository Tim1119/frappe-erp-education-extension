 ...
 */
export default function Pager({ rows, count, page, setPage, pageSize = PAGE_SIZE }) {
  ...
  return (
    <div className="pager">
      <span className="tnum muted">
        {total ? `${Math.min(start + 1, total)}–${Math.min(start + pageSize, total)} of ${total}` : '0 records'}
      </span>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        <button className="pg-btn" disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>
          <ChevronLeft size={15} />
        </button>
        <span style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>{page} / {totalPages}</span>
