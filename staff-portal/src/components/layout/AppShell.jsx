...
//                 <div
//                   className="rowmenu"
//                   style={{ right: 0, top: 46, minWidth: 240 }}
//                 >
...
export default function AppShell() {
  ...
  // Render a navigation item (could be regular or dropdown)
  const renderNavItem = (item) => {
    // Check if it's a dropdown item (has children)
    if (item.children) {
      ...
      return (
        <div key={item.key} className="nav-dropdown">
          <div
            className={cx("nav-item", hasActiveChild && "active")}
        ...
        <aside className={cx("sidebar", sideOpen && "open")}>
          ...
          <div className="side-scroll">
            {menu.map((grp) => (
              <div key={grp.g}>
                <div className="nav-label">{grp.g}</div>
                {grp.items.map((item) => renderNavItem(item))}
              </div>
            ))}
          </div>
