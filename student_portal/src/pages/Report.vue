		...
		>
			...
			>
				...
				<div v-if="report.total_marks_obtained" class="mb-4">
					...
					<div class="w-full bg-gray-200 rounded-full h-2">
						<div
							...
							"
							:style="{
								width: `${(report.total_marks_obtained / report.total_max_marks) * 100}%`,
							}"
						></div>
					</div>
					...
					<div class="text-right mt-1">
						<span class="text-xs text-gray-500">
							{{
								Math.round(
									(report.total_marks_obtained / report.total_max_marks) * 100,
								)
							}}%
						</span>
					</div>
				...
				<div class="grid grid-cols-3 gap-2 mb-4">
					...
					<div
						v-if="report.class_arm_position"
...
// const getPdfLink = (doctype, docname, printFormat = "Standard", noLetterhead = 0, pdfGenerator = "chrome") => {
//   return `/api/method/frappe.utils.print_format.download_pdf?doctype=${encodeURIComponent(doctype)}&name=${encodeURIComponent(docname)}&format=${encodeURIComponent(printFormat)}&no_letterhead=${noLetterhead}&pdf_generator=${pdfGenerator}`;
// }

// const getPdfLink = (doctype, docname, printFormat = "Standard", noLetterhead = 0) => {
//   return `/api/method/education_extension.education_extension.api.download_pdf_chrome?doctype=${encodeURIComponent(doctype)}&name=${encodeURIComponent(docname)}&format=${encodeURIComponent(printFormat)}&no_letterhead=${noLetterhead}`;
// }
