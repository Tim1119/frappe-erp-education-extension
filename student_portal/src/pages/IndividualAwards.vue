<template>
	<div class="min-h-screen bg-gray-50 px-6 py-8">
		...
		<!-- Awards Grid -->
		<div
			v-else-if="filteredAwards.length > 0"
...
const yearOptions = computed(() => {
	...
	return [{ label: "All Years", value: "", onClick: () => (filterYear.value = "") }].concat(
		years.map((y) => ({ label: y, value: y, onClick: () => (filterYear.value = y) })),
	);
});
...
const categoryOptions = [
	...
	{ label: "Sports", value: "Sports", onClick: () => (filterCategory.value = "Sports") },
	{
		label: "Leadership",
		value: "Leadership",
		onClick: () => (filterCategory.value = "Leadership"),
	},
	{
		label: "Behavioural",
		value: "Behavioural",
		onClick: () => (filterCategory.value = "Behavioural"),
	},
];

const filteredAwards = computed(() => {
	return allAwards.value.filter(
		(a) =>
