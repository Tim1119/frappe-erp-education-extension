			...
			<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				<div>
					...
					<Dropdown :options="categoryOptions" v-model="filterCategory">
						<template #default="{ open }">
							<Button
								:label="filterCategory || 'All Types'"
								class="w-full justify-between"
							>
								<template #suffix>
									<FeatherIcon
				...
				<div>
					...
					<Dropdown :options="studentGroupOptions" v-model="filterStudentGroup">
						<template #default="{ open }">
							<Button
								:label="filterStudentGroup || 'All Classes'"
								class="w-full justify-between"
							>
								<template #suffix>
									<FeatherIcon
...
const filteredCertificates = computed(() => {
	return allCertificates.value.filter((cert) => {
		...
		const groupMatch =
			!filterStudentGroup.value || cert.studentGroup === filterStudentGroup.value;
