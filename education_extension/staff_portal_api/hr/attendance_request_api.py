...
def get_attendance_requests(page=1,page_size=20,search=None,employee=None,company=None,department=None,reason=None,date_from=None,date_to=None):
    ...
    rows=frappe.get_all("Attendance Request",fields=fields,filters=filters,or_filters=ors,order_by="from_date desc, name desc",start=(page-1)*page_size,page_length=page_size)
    ...
    total=len(frappe.get_all("Attendance Request",filters=filters,or_filters=ors,pluck="name",limit_page_length=0));return {"rows":rows,"count":total,"page":page,"page_size":page_size,"total_pages":((total+page_size-1)//page_size if page_size else 1)}
@frappe.whitelist()
...
@frappe.whitelist()
def get_employees():return _safe(lambda:frappe.get_all("Employee",fields=["name","employee_name","company","department","status"],order_by="employee_name",limit_page_length=500))
@frappe.whitelist()
