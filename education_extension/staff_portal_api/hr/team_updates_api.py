...
def get_team_updates(page=1, page_size=20, search=None, sender=None, date_from=None, date_to=None, group=None):
    ...
    if group:
        summaries = frappe.get_list("Daily Work Summary", filters={"daily_work_summary_group": group}, pluck="name", limit_page_length=0)
        if not summaries:
    ...
    rows = frappe.get_list("Communication", fields=fields, filters=filters, or_filters=or_filters, order_by="creation desc", start=(page - 1) * page_size, page_length=page_size)
    for row in rows:
    ...
    total = len(frappe.get_list("Communication", filters=filters, or_filters=or_filters, pluck="name", limit_page_length=0))
...
def get_senders():
    try:
        return frappe.get_list("Communication", fields=["sender as name"], filters={"reference_doctype": "Daily Work Summary", "sent_or_received": "Received"}, group_by="sender", order_by="sender", limit_page_length=500)
