...
frappe.ui.form.on('School Club', {
	refresh: function(frm) {
		// Add custom buttons
		if (!frm.is_new()) {
			frm.add_custom_button(__('View Available Students'), function() {
				show_available_students(frm);
			}, __('Students'));
			
			frm.add_custom_button(__('View Available Instructors'), function() {
				show_available_instructors(frm);
			}, __('Instructors'));
			
			frm.add_custom_button(__('Club Summary'), function() {
	...
	academic_year: function(frm) {
		...
		if ((frm.doc.club_instructors && frm.doc.club_instructors.length > 0) || 
		    (frm.doc.club_members && frm.doc.club_members.length > 0)) {
			frappe.confirm(
				__('Changing Academic Year will clear all instructors and members. Do you want to continue?'),
				function() {
...
function highlight_primary_instructor(frm) {
	if (frm.fields_dict['club_instructors']) {
		frm.fields_dict['club_instructors'].grid.wrapper.find('.grid-body .rows').each(function() {
...
frappe.ui.form.on('Club Member', {
	student: function(frm, cdt, cdn) {
		...
		if (row.student && frm.doc.academic_year) {
			// Fetch student name
			frappe.db.get_value('Student', row.student, ['student_name', 'first_name', 'middle_name', 'last_name'], function(r) {
				if (r) {
		...
		callback: function(r) {
			if (r.message && r.message.length > 0) {
				...
				if (available.length > 0) {
					...
					frappe.msgprint(__('All available students have already been added to this club'));
				}
			} else {
...
function show_student_dialog(frm, students) {
	...
	function render_student_table(filtered_students) {
		...
		html += '<table class="table table-bordered table-hover" id="students-table"><thead><tr>' +
		...
		if (filtered_students.length === 0) {
			html = '<div class="text-center text-muted" style="padding: 40px;">No students found matching your search</div>';
		}
	...
	function filter_student_table(search_term) {
		...
		let filtered = students.filter(function(student) {
			return student.name.toLowerCase().includes(search_term) ||
				   (student.student_name && student.student_name.toLowerCase().includes(search_term)) ||
				   (student.student_email_id && student.student_email_id.toLowerCase().includes(search_term));
		});
		...
		callback: function(r) {
			if (r.message && r.message.length > 0) {
				...
				if (frm.doc.club_instructors) {
					already_added = frm.doc.club_instructors.map(i => i.instructor).filter(Boolean);
				}
				...
				if (available.length > 0) {
					...
					frappe.msgprint(__('All available instructors have already been added to this club'));
				}
			} else {
...
function show_instructor_dialog(frm, instructors) {
	...
	function render_instructor_table(filtered_instructors) {
		let html = '<div style="max-height: 500px; overflow-y: auto;">';
		html += '<table class="table table-bordered table-hover" id="instructors-table"><thead><tr>' +
		...
		if (filtered_instructors.length === 0) {
			html = '<div class="text-center text-muted" style="padding: 40px;">No instructors found matching your search</div>';
		}
	...
	function filter_instructor_table(search_term) {
		...
		let filtered = instructors.filter(function(instructor) {
			return instructor.name.toLowerCase().includes(search_term) ||
				   (instructor.instructor_name && instructor.instructor_name.toLowerCase().includes(search_term)) ||
				   (instructor.department && instructor.department.toLowerCase().includes(search_term));
		});
...
function show_club_summary(frm) {
	let primary_instructor = frm.doc.club_instructors ? frm.doc.club_instructors.find(i => i.is_primary) : null;
	...
	let active_members = frm.doc.club_members ? frm.doc.club_members.filter(m => m.status === 'Active').length : 0;
