...
def generate_class_results(docname):
	...
	if not (
		generator_doc.assessment_group
		and generator_doc.academic_year
		and generator_doc.academic_term
		and generator_doc.student_group
	):
	...
	students = frappe.get_all(
		"Student Group Student", filters={"parent": generator_doc.student_group}, fields=["student"]
	)
	...
	for entry in students:
		...
		result_doc.date_of_result_issue = generator_doc.date_of_result_issue
...
def populate_student_result(doc):
	...
	try:
		...
		doc.next_term_start_date = school_settings.next_term_start_date
	except Exception as e:
		frappe.log_error(f"Error fetching next_term_start_date from School Settings: {str(e)}")
		doc.next_term_start_date = None
	...
	sgs = frappe.get_all("Student Group Student", filters={"student": doc.student}, fields=["parent"])
	...
	if sgs:
		...
		doc.number_of_students_in_class_group = frappe.db.count(
			"Student Group Student", filters={"parent": student_group_name}
		)
		...
		doc.number_of_students_in_class = frappe.db.count(
			"Program Enrollment",
			filters={"program": program, "academic_year": doc.academic_year, "docstatus": 1},
		)
	else:
    ...
    """,
		(doc.student, doc.academic_year, doc.assessment_group),
		as_dict=True,
	)
	...
	if detailed_results:
		...
		for row in detailed_results:
			...
			if course not in course_details:
				course_details[course] = {"details": [], "total_score": row.total_score, "grade": row.grade}
		...
		for subject_row in doc.subjects:
			...
			if previous_term_1:
				try:
					result_name = frappe.db.get_value(
						"School Term Result",
						{"student": doc.student, "assessment_group": previous_term_1},
						"name",
					)
			...
			if previous_term_2:
				try:
					result_name = frappe.db.get_value(
						"School Term Result",
						{"student": doc.student, "assessment_group": previous_term_2},
						"name",
					)
