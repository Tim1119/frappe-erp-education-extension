frappe.pages["student-birthdays"].on_page_load = function (wrapper) {
	...
	page.add_field({
		...
		label: __("View By"),
		fieldtype: "Select",
		options: ["Upcoming Birthdays", "Specific Month", "All Students"],
		...
		if (students.length === 0) {
			...
			if (view_type === "Specific Month") {
				...
				if (today_birthdays.length > 0) {
					html +=
						'<h3 style="color: #667eea; margin-top: 20px;">🎉 Today\'s Birthdays</h3>';
					html += '<div class="row">';
				...
				if (upcoming_birthdays.length > 0) {
					html +=
						'<h3 style="color: #764ba2; margin-top: 30px;">📅 Upcoming Birthdays</h3>';
					html += '<div class="row">';
