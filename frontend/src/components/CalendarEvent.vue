<template>
  <div style="position: relative;">
    ...
    >
      <p style="font-size:0.7rem;font-weight:600;color:#1e293b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin:0;">
        {{ event.title || event.course }}
      </p>
      <p v-if="event.from_time" style="font-size:0.65rem;color:#64748b;margin:0;white-space:nowrap;">
        ...
        >
          ...
          <div
            :style="{
              ...
              borderBottom: '1px solid ' + (colorStyles[event?.color]?.border || '#2563eb'),
              padding: '12px 16px 10px',
              borderRadius: '12px 12px 0 0',
            }"
          >
            <p style="margin:0;font-size:0.8125rem;font-weight:700;color:#1e293b;line-height:1.3;">
              {{ event.title || event.course }}
              <span v-if="event.instructor" style="font-weight:400;color:#475569;"> by {{ event.instructor }}</span>
            </p>
          </div>
          ...
          <div style="padding: 12px 16px 14px; display:flex; flex-direction:column; gap:10px;">
            ...
            <div v-if="event.schedule_date" style="display:flex;gap:10px;align-items:center;">
              <svg style="width:15px;height:15px;color:#6b7280;flex-shrink:0;" viewBox="0 0 20 20" fill="#6b7280">
                <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"/>
              </svg>
              <span style="font-size:0.8125rem;color:#374151;">{{ formatDate(event.schedule_date) }}</span>
            </div>

            <div v-if="event.from_time" style="display:flex;gap:10px;align-items:center;">
              <svg style="width:15px;height:15px;flex-shrink:0;" viewBox="0 0 20 20" fill="#6b7280">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/>
              </svg>
              <span style="font-size:0.8125rem;color:#374151;">{{ event.from_time?.substring(0,5) }} – {{ event.to_time?.substring(0,5) }}</span>
            </div>

            <div v-if="event.instructor" style="display:flex;gap:10px;align-items:center;">
              <svg style="width:15px;height:15px;flex-shrink:0;" viewBox="0 0 20 20" fill="#6b7280">
                <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"/>
              </svg>
              <span style="font-size:0.8125rem;color:#374151;">{{ event.instructor }}</span>
            </div>

            <div v-if="event.room" style="display:flex;gap:10px;align-items:center;">
              <svg style="width:15px;height:15px;flex-shrink:0;" viewBox="0 0 20 20" fill="#6b7280">
              ...
              <span style="font-size:0.8125rem;color:#374151;">Room: {{ event.room }}</span>
            </div>
...
.popup-leave-active { transition: opacity 0.1s ease, transform 0.1s ease; }
.popup-enter-from  { opacity: 0; transform: translateY(4px); }
.popup-leave-to    { opacity: 0; transform: translateY(4px); }
</style>