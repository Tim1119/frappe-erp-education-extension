<template>
  <div class="report-page">
      ...
      <div class="filter-grid">
        ...
        <div class="filter-field">
          ...
          <div class="filter-dropdown-wrap" ref="classRef">
            <button class="filter-btn" @click.stop="toggleFilter('class')">
              ...
              <svg viewBox="0 0 20 20" fill="currentColor" class="chevron" :class="{ open: openFilter === 'class' }">
                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/>
              </svg>
            </button>
            <Teleport to="body">
              <div v-if="openFilter === 'class'" class="report-filter-menu" :style="filterMenuStyle" @click.stop>
                <button class="report-filter-item" :class="{ active: !filterProgram }" @click="filterProgram = ''; openFilter = null">All Classes</button>
                <button v-for="p in programOptions" :key="p" class="report-filter-item" :class="{ active: filterProgram === p }" @click="filterProgram = p; openFilter = null">{{ p }}</button>
              </div>
            </Teleport>
          </div>
        </div>
        ...
        <div class="filter-field">
          ...
          <div class="filter-dropdown-wrap" ref="yearRef">
            ...
            <Teleport to="body">
              <div v-if="openFilter === 'year'" class="report-filter-menu" :style="filterMenuStyle" @click.stop>
                <button class="report-filter-item" :class="{ active: !filterYear }" @click="filterYear = ''; openFilter = null">All Years</button>
                <button v-for="y in yearOptions" :key="y" class="report-filter-item" :class="{ active: filterYear === y }" @click="filterYear = y; openFilter = null">{{ y }}</button>
              </div>
            </Teleport>
          </div>
        </div>
        ...
        <div class="filter-field">
          ...
          <div class="filter-dropdown-wrap" ref="termRef">
            <button class="filter-btn" @click.stop="toggleFilter('term')">
              ...
              <svg viewBox="0 0 20 20" fill="currentColor" class="chevron" :class="{ open: openFilter === 'term' }">
                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/>
              </svg>
            </button>
            <Teleport to="body">
              <div v-if="openFilter === 'term'" class="report-filter-menu" :style="filterMenuStyle" @click.stop>
                <button class="report-filter-item" :class="{ active: !filterTerm }" @click="filterTerm = ''; openFilter = null">All Terms</button>
                <button v-for="t in termOptions" :key="t" class="report-filter-item" :class="{ active: filterTerm === t }" @click="filterTerm = t; openFilter = null">{{ t }}</button>
              </div>
            </Teleport>
          </div>
        </div>
      ...
      <div v-for="report in filteredReports" :key="report.name" class="report-card">
        ...
        <div v-if="report.total_marks_obtained" class="card-progress">
          <div class="progress-row">
            ...
            <span class="progress-value">{{ report.total_marks_obtained }}/{{ report.total_max_marks }}</span>
          </div>
          <div class="progress-track">
            <div
              class="progress-fill"
              :class="progressClass(report.total_marks_obtained, report.total_max_marks)"
              :style="{ width: pct(report.total_marks_obtained, report.total_max_marks) + '%' }"
            ></div>
          </div>
          <div class="progress-pct">{{ pct(report.total_marks_obtained, report.total_max_marks) }}%</div>
        </div>
        ...
        <div class="card-footer">
          <button class="btn-print" @click="printReport(report)" :disabled="printingReports[report.name]">
    ...
    <div v-else class="empty-card">
      ...
      <p class="empty-sub">{{ hasFilters ? 'No reports match your current filters.' : 'No reports available yet.' }}</p>
...
function isSecondaryProgram(program) {
  ...
  return ['jss', 'ss', 'secondary', 'high school', 'senior','junior', 'junior secondary'].some(k => lower.includes(k))
}
...
/* Report Card */
.report-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 1.25rem; display: flex; flex-direction: column; gap: 1rem; transition: box-shadow 0.2s; }
.report-card:hover { box-shadow: 0 4px 16px rgba(0,0,0,0.08); }

.card-header { display: flex; flex-direction: column; gap: 0.25rem; }
.card-title { font-size: 0.9375rem; font-weight: 700; color: #111; margin: 0; line-height: 1.4; }
.card-sub { font-size: 0.8125rem; color: #6b7280; margin: 0; }

.card-progress { display: flex; flex-direction: column; gap: 0.375rem; }
.progress-row { display: flex; justify-content: space-between; }
.progress-label { font-size: 0.8125rem; font-weight: 500; color: #374151; }
.progress-value { font-size: 0.8125rem; font-weight: 600; color: #111; }
...
.progress-pct { font-size: 0.75rem; color: #9ca3af; text-align: right; }

.card-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem; }
.stat-box { background: #f9fafb; border-radius: 8px; padding: 0.625rem; text-align: center; }
.stat-val { font-size: 1rem; font-weight: 700; color: #111; }
.stat-lbl { font-size: 0.7rem; color: #6b7280; margin-top: 2px; }

.card-footer { border-top: 1px solid #f3f4f6; padding-top: 0.75rem; }
.btn-print { width: 100%; display: flex; align-items: center; justify-content: center; gap: 0.4rem; padding: 0.5rem; font-size: 0.875rem; font-weight: 500; color: #374151; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; cursor: pointer; font-family: inherit; transition: background 0.15s; }
.btn-print:hover { background: #111; color: #fff; border-color: #111; }
...
.report-filter-item:hover { background: #f9fafb; }
.report-filter-item.active { background: #f0f4ff; color: #1d4ed8; font-weight: 600; }
</style>