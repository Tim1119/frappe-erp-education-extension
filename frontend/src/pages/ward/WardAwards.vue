	...
	<div class="awards-page">
		...
		<div class="section">
			...
			<div v-else-if="filteredIndividual.length > 0" class="awards-grid">
				<div v-for="award in filteredIndividual" :key="award.name" class="award-card">
					...
					<div class="award-body">
						...
						<p v-if="award.certificate_date" class="award-date">
							{{ formatDate(award.certificate_date) }}
						</p>
						<p v-if="award.description" class="award-desc">
							{{ stripHtml(award.description) }}
						</p>
						<div class="award-footer">
							<span v-if="award.certificate_type" class="award-badge">{{
								award.certificate_type
							}}</span>
							<span v-if="award.academic_year" class="award-year">{{
								award.academic_year
							}}</span>
							<a
								v-if="award.certificate_file"
			...
			<div v-else-if="!data.loading" class="empty-card">
				...
				<p class="empty-sub">
					{{
						hasFilters ? 'No awards match your filters.' : 'No individual awards yet.'
					}}
				</p>
				<button
					v-if="hasFilters"
		...
		<div class="section" v-if="!data.loading && data.data?.general?.length">
			...
			<div class="awards-grid">
				<div
					v-for="award in data.data.general"
					:key="award.name"
				...
				>
					...
					<div class="award-body">
						...
						<div class="award-footer">
							<span
								v-if="award.certificate_type"
								class="award-badge general-badge"
								>{{ award.certificate_type }}</span
							>
							<span v-if="award.student_group" class="award-year">{{
								award.student_group
							}}</span>
							<a
								v-if="award.certificate_file"
...
const filteredIndividual = computed(() => {
	return individual.value.filter(
		(a) =>
