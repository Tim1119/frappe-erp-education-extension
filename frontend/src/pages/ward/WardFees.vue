			...
			<div class="fee-summary">
				<div class="fee-sum-card outstanding">
					...
					<span class="sum-amount"
						>₦{{ formatAmount(data.data.summary.total_outstanding) }}</span
					>
				</div>
				<div class="fee-sum-card paid">
					<span class="sum-label">Total Paid</span>
					<span class="sum-amount"
						>₦{{ formatAmount(data.data.summary.total_paid) }}</span
					>
				</div>
			</div>
	...
	return [...data.data.invoices]
		.map((inv) => ({
			...
			amount_display: `${inv.currency || '₦'} ${formatAmount(
				inv.status === 'Paid'
					? inv.grand_total
					: inv.outstanding_amount || inv.grand_total,
			)}`,
		}))
