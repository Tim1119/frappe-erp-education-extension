		...
		<template v-else-if="data.data">
			...
			<div class="progress-section">
				<div class="progress-track">
					<div
						...
						:class="{
							...
							orange:
								data.data.summary.percentage >= 50 &&
				...
				<p class="progress-note">
					Based on last {{ data.data.summary.total }} school days
				</p>
			</div>
					...
					<template #day-content="{ day }">
						<div class="a-day" :class="{ 'a-day--out': !day.inMonth }">
							...
							<div v-if="getRecord(day.date)" class="a-dot-wrap">
								<span
									class="a-dot"
									:class="dotClass(getRecord(day.date).status)"
								></span>
								<span
									class="a-label"
									:class="labelClass(getRecord(day.date).status)"
								>
...
.stat-card.dark .stat-val,
.stat-card.dark .stat-lbl {
	color: #fff;
}
.stat-val {
	font-size: 1.5rem;
...
.cal-wrap :deep(.vc-arrow:hover) {
	background: #e5e7eb;
}

.cal-wrap :deep(.vc-weekdays) {
	border-bottom: 1px solid #e5e7eb;
}
...
.cal-wrap :deep(.vc-weekday:last-child) {
	border-right: none;
}

.cal-wrap :deep(.vc-weeks) {
	padding: 0;
